...
public final class QueryUtils {
    ...
    public static ArrayList<Quake> extractEarthquakes() {
        ...
        try {
            //converts the string we have received like response into a JSONObject
            ...
            // Loop through each feature in the array until end lenght reached
            for (int i = 0; i < jFeatures.length(); i++) {

                JSONObject currentFeatures = jFeatures.getJSONObject(i);

                JSONObject JProperties = currentFeatures.getJSONObject("properties");

                // build up a list of Earthquake objects with the corresponding data.

                // Extract the value for the key called "mag"
                double magnitude = JProperties.getDouble("mag");

                String location = JProperties.getString("place");

                // Extract the value for the key called "time"
                long time = JProperties.getLong("time");
                Quake earthquakeItem = new Quake(magnitude,location,time);
